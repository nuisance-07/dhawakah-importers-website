    ...
    <section className="py-24 bg-dark-surface relative">
      <div className="container mx-auto px-4 md:px-8">
        <ScrollReveal className="text-center mb-16">
          ...
          <h2 className="font-heading text-4xl md:text-5xl font-bold text-white uppercase">Our Services</h2>
        </ScrollReveal>
          ...
          {services.map((service, index) => (
            <ScrollReveal key={index} delay={service.delay} direction="up">
              <Link href={service.href} className="block group h-full">
                <div className="bg-black/50 border border-white/5 p-10 h-full hover:bg-black/80 hover:border-primary/50 transition-all duration-300 relative overflow-hidden flex flex-col items-start rounded-sm">
                  <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mb-8 group-hover:bg-primary/20 transition-colors">
                    <service.icon size={32} className="text-secondary group-hover:text-primary transition-colors" />
                  </div>
                  <h3 className="font-heading text-2xl font-semibold text-white mb-4 group-hover:text-primary transition-colors">{service.title}</h3>
